...
int main() {
  ...
  while (true) {
    ...
    if (action == '1') {
      ...
      while (1) {
        cout << "(A)dd Customer, Enter a (S)ale, (V)iew Customer, or (M)ain "
                "Menu? ";
        ...
        if (sales_choice == 'A' || sales_choice == 'a') {
          ...
          if (cus_end - cus_begin == 0) {
            ...
            cout << customer_choice.name() << " <" << customer_choice.email()
                 << "> "
                 << "Phone: " << customer_choice.phone();
            cout << "\n";
            cout << "Order History\n"
                    "Item                   Price  Quantity   Total\n";
            for (auto i = customer_choice.purchase_begin();
                 i != customer_choice.purchase_end(); i++) {
              cout << i->item() << "     " << i->cost() << "    " << i->qty()
                   << "    " << i->total() << "\n";
            }
          }
          ...
          if (cus_end - cus_begin == 0) {
            ...
            cust_choice = stoi(temp);
            int counter = 0;
            for (auto i = company_database.customer_begin();
                 i != company_database.customer_end(); i++) {
